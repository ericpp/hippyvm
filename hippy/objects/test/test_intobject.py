...
if sys.maxint == 2**31-1:
    ...
    R1 = 9.22337203685e+18      # approximately sys.maxint + 1
    R2 = 1.84467440737e+19      # approximately 2 * sys.maxint
...
class TestIntObject(BaseTestInterpreter):
    ...
    def test_uplusplus(self):
        ...
        output = self.run('$a = %d;\necho ++$a;' % sys.maxint)
        assert self.space.is_w(
            self.space.lt(
                self.space.sub(output[0], self.space.newfloat(sys.maxint+1)),
                self.space.newfloat(sys.maxint * 1E-12)),
            self.space.w_True)
